...
namespace Devdog.InventoryPro.Editors {
    public class CraftingBlueprintEditor : ScriptableObjectEditorCrud<CraftingBlueprint> {
        ...
        void UpdateBlueprintID(CraftingBlueprint selectedBlueprint) {
            // RenameScriptableObjectIfNeeded(selectedBlueprint, selectedBlueprint.ID + "_" + selectedBlueprint.name.Replace(",", "_").Replace(" ", "_"));

            if (category != null) {
                selectedBlueprint.ID = category.blueprints.ToList().IndexOf(selectedBlueprint);
            }

            int categoryID = category != null ? category.ID : 0;
            RenameScriptableObjectIfNeeded(selectedBlueprint, categoryID + "_" + selectedBlueprint.name.Replace(",", "_").Replace(" ", "_"));
        }
